...
export default function CardTeam({ item }: { item: any }) {
  ...
  return (
    <div onClick={() => router.push("/teamDetail?id=" + item.id )} key={item.id}>
      <div className="flex flex-col justify-center items-center bg-blue-800 w-[170px] h-[200px] mr-5 ml-5 text-white p-5 rounded-2xl cursor-pointer">
        <Image
          className="rounded-full"
          src={item.flag !== null ? item.flag : default_escudo}
          width={50}
          height={50}
